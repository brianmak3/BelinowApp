...
class CategoryProducts extends Component {
            ...
            <Animatable.View animation={'fadeInDown'} delay={500} duration={300}>
                <View style={filterStyles.categoryFilters}>
                    <View style={filterStyles.filterName}>
                        <Text style={filterStyles.filterNameText}>{count} items</Text>
                    </View>
    ...
    _getProducts() {
        const { query } = this.state
        this.props.ProductAction.getProducts(query)
    }

    _renderProducts() {
        const { products } = this.props
        const { key, columns } = this.state
        return (
            <Animatable.View
                animation={key > 2 ? '' : 'fadeInUp'}
                duration={500}
                key={key}>
                <FlatList
                    data={products.data}
            ...
            <View style={styles.container}>
                ...
                <ScrollView showsHorizontalScrollIndicator={false} showsVerticalScrollIndicator={false} contentInsetAdjustmentBehavior={'never'}>
                    <View style={styles.tabContent}>
                        ...
                        {!loading && data ? this._renderProducts() : null}
                    </View>
                </ScrollView>
            </View>
        )
